...
InputParameters validParams<BubblesPPAction>()
{
  ...
  params.addParam<std::vector<OutputName> >("show_total_concentrations", "Where to output concentration postprocessors. Not calculated if empty");
  params.addParam<std::vector<OutputName> >("show_total_atoms", "Where to output concentration postprocessors. Not calculated if empty");

  return params;
}

BubblesPPAction::BubblesPPAction(const std::string & name, InputParameters params) :
